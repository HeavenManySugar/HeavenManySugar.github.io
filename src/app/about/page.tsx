  ...
  const educationItems = [
    {
      ...
      title: "國立臺北科技大學",
      subtitle: "資訊工程學系 四年級",
      description: "學習軟體工程、資料結構、演算法與人工智慧，積極參與系學會活動。"
    },
    {
      year: "2018-2021",
      title: "國立臺南高級工業職業學校",
      subtitle: "資訊科",
      description: "奠定程式設計基礎，擔任網頁設計社副社長。"
    }
  ];

  const awards = [
    {
      title: "教育部全國大專院校人工智慧競賽",
      subtitle: "根據區域微氣候資料預測發電量",
      award: "佳作 - 第9名 / 934隊",
      year: "2024"
    },
    {
  ...
  const techStack = [
    {
      ...
      description: "深入學習現代前端技術棧，專注於組件化開發與性能優化。"
    },
    {
      title: "後端開發",
      skills: ["Node.js", "Express.js", "Golang", "PostgreSQL"],
      description: "掌握後端開發，能夠獨立完成全端專案。"
    },
    {
      title: "設計思維",
      skills: ["Figma", "Design System", "UX/UI", "Prototyping"],
      description: "注重使用者體驗，實現技術與設計的完美結合。"
    },
    {
      title: "人工智慧",
      skills: ["Python", "Machine Learning", "TensorFlow", "Data Analysis"],
      description: "透過實際競賽深入學習機器學習與深度學習技術。"
    }
  ];
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-gray-950">
      ...
      <section className="container mx-auto px-6 lg:px-8 py-12">
        ...
        >
          {[
            { label: "學校", value: "國立臺北科技大學" },
            { label: "系別", value: "資訊工程學系" },
            { label: "年級", value: "四年級" }
          ].map((item) => (
            <MotionWrapper
              key={item.label}
          ...
          <div className="space-y-6">
            {educationItems.map((item, i) => (
              <MotionWrapper
                key={i}
                variants={itemVariants}
                className="flex gap-6 pb-6 border-b border-gray-200 dark:border-gray-800 last:border-b-0"
              >
                <div className="text-sm font-semibold text-blue-600 dark:text-blue-400 min-w-fit">{item.year}</div>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {awards.map((award, i) => (
              <MotionWrapper
                key={i}
                variants={itemVariants}
                className="bg-gradient-to-br from-blue-50 to-blue-50 dark:from-blue-950/20 dark:to-blue-900/20 p-6 rounded-lg border border-blue-200 dark:border-blue-800"
              >
                <div className="flex items-start justify-between mb-3">
                  <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{award.title}</h3>
                  <span className="text-sm text-blue-600 dark:text-blue-400 font-medium">{award.year}</span>
                </div>
                <p className="text-sm text-gray-600 dark:text-gray-400 mb-3">{award.subtitle}</p>
                <p className="text-blue-600 dark:text-blue-400 font-semibold">{award.award}</p>
              </MotionWrapper>
            ))}
          </div>
        </MotionWrapper>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {techStack.map((tech, i) => (
              <MotionWrapper
                ...
                className="bg-gray-50 dark:bg-gray-900 p-6 rounded-lg border border-gray-200 dark:border-gray-800"
              >
                <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-3">{tech.title}</h3>
                <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">{tech.description}</p>
                <div className="flex flex-wrap gap-2">
                  {tech.skills.map((skill) => (
                    <span
                      key={skill}
                      className="px-3 py-1 bg-blue-100 dark:bg-blue-950 text-blue-700 dark:text-blue-400 text-xs font-medium rounded-full"
                    >
                      {skill}
                    </span>
                  ))}
