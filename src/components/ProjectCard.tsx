...
export default function ProjectCard({
    ...
    return (
        <div className={`bg-white dark:bg-gray-700 rounded-xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 group ${isLarge ? '' : 'shadow-md hover:shadow-lg'}`}>
            <div className={`${isLarge ? 'h-48' : 'h-32'} bg-gradient-to-r ${gradientFrom} ${gradientTo} flex items-center justify-center relative overflow-hidden`}>
                ...
                <span className={`${isLarge ? 'text-4xl' : 'text-2xl'} text-white relative z-10 group-hover:scale-110 transition-transform duration-300`}>
                    {icon}
                </span>
            </div>
            <div className={isLarge ? 'p-6' : 'p-4'}>
                <h3 className={`${isLarge ? 'text-xl' : 'text-lg'} font-semibold text-gray-800 dark:text-white mb-2 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors duration-300`}>
                    {title}
                </h3>
                <p className={`text-gray-600 dark:text-gray-300 mb-4 leading-relaxed ${isLarge ? '' : 'text-sm mb-3'}`}>
                    {description}
                </p>
                <div className={`flex flex-wrap gap-2 mb-4 ${isLarge ? '' : 'gap-1 mb-3'}`}>
                    {technologies.map((tech, index) => (
                        <span
                            key={index}
                            className={`${getTechColor(tech)} px-2 py-1 rounded-full font-medium transition-all duration-200 hover:scale-105 ${isLarge ? 'text-xs' : 'text-xs'}`}
                        >
                            {tech}
                        </span>
                    ))}
                ...
                <div className="flex gap-3">
                    {projectUrl && (
                        <a
                            ...
                            className={`text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 hover:underline transition-all duration-200 font-medium flex items-center gap-1 ${isLarge ? 'text-sm' : 'text-xs'}`}
                        >
                            查看專案
                    ...
                    <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className={`text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-300 hover:underline transition-all duration-200 font-medium flex items-center gap-1 ${isLarge ? 'text-sm' : 'text-xs'}`}
                    >
                        GitHub
